...
WalkEngine::WalkEngine(std::string wkfile, std::shared_ptr<seurobot::SeuRobot> robot): robot_(robot)
{
    boost::property_tree::ptree pt;
    bool ret = basic_parser::parse_file(wkfile, pt);
    if(!ret) return;
    XOffset_ = pt.get<double>("XOffset");
    YOffset_ = pt.get<double>("YOffset");
    DOffset_ = pt.get<double>("DOffset");

    xrange_ = basic_parser::get_config_vector<double, 2>(pt, "x");
    yrange_ = basic_parser::get_config_vector<double, 2>(pt, "y");
    drange_ = basic_parser::get_config_vector<double, 2>(pt, "dir");
    /**
     * Model leg typical length between
     * each rotation axis
     */
    params_.distHipToKnee = robot_->get_bone("rthigh")->length;
    params_.distKneeToAnkle = robot_->get_bone("rshank")->length;
    params_.distAnkleToGround = robot_->get_bone("rfoot1")->length;
    params_.distFeetLateral = robot_->get_bone("hip")->length;
    /**
...
std::vector<common::msg::BodyAngles> WalkEngine::runWalk(Eigen::Vector3d p, int steps, double& phase)
{
    ...
    std::vector<common::msg::BodyAngles> ret;
