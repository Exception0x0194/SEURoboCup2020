...
JointPtr parse_joint(const std::string &j_name, const bpt::ptree &pt,
                     BoneMap &BoneMap, JointMap &JointMap);
BonePtr parse_bone(const bpt::ptree &pt, BoneMap &BoneMap,
                   JointMap &JointMap);
...
bool parse(std::string act_file, ActMap &acts, PosMap &poses)
{
    ...
    for (auto &pos : pos_pt) {
        ...
        for (auto &info : pos.second) {
            t_pos.pose_info[get_motion_by_name(info.first)] = get_pose_from_string(info.second.data());
        }
        poses[t_pos.name] = t_pos;
    }

    RobotAct t_act;
    RobotOnePos t_one_pos;
...
SeuRobot::SeuRobot(const std::string &robot_file, const std::string &offset_file)
{
    bpt::ptree pt;
    start_id_ = 0xff;
...
TransformMatrix SeuRobot::get_foot_mat_from_pose(const RobotPose &pose, bool left)
{
...
TransformMatrix SeuRobot::get_body_mat_from_pose(const RobotPose &pose)
{
...
bool SeuRobot::arm_inverse_kinematics(const Vector3d &hand, vector<double> &deg)
{
    double x = hand[0];
...
TransformMatrix SeuRobot::leg_forward_kinematics(vector<double> degs, bool left)
{
    double sg = (left ? 1.0 : -1.0);
    ...
    body = foot * TransformMatrix(0, 0, foot_length) * TransformMatrix(-degs[5], 'x') *
           TransformMatrix(-degs[4], 'y') * TransformMatrix(0, 0, shank_length) *
           TransformMatrix(-degs[3], 'y') * TransformMatrix(0, 0, thigh_length) *
           TransformMatrix(-degs[2], 'y') * TransformMatrix(-degs[1], 'x') * TransformMatrix(-degs[0], 'z') *
           TransformMatrix(0, -sg * hip_distance / 2.0, bone_map_["rhip2"]->length);
    return body;
}

bool SeuRobot::leg_inverse_kinematics(const TransformMatrix &body,
...
void SeuRobot::set_degs(const std::map<int, float> &jdmap)
{
...
void SeuRobot::set_head(const common::msg::HeadAngles &head)
{
    std::lock_guard<std::mutex> lk(robot_mtx_);
...
void SeuRobot::set_body(const common::msg::BodyAngles &body)
{
    std::lock_guard<std::mutex> lk(robot_mtx_);
...
std::vector<double> SeuRobot::get_foot_degs(int support)
{
    std::lock_guard<std::mutex> lk(robot_mtx_);
...
BonePtr parse_bone(const bpt::ptree &pt, BoneMap &BoneMap, JointMap &JointMap)
{
    BonePtr b = std::make_shared<Bone>();
...
JointPtr parse_joint(const std::string &j_name, const bpt::ptree &pt, BoneMap &BoneMap, JointMap &JointMap)
{
